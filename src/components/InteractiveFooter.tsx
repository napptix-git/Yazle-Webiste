...
class PerformanceMonitorClass implements PerformanceMonitor {
  ...
  applyQualitySettings() {
    // This will be handled by the React component state
...
const InteractiveFooter: React.FC = () => {
  ...
  const [gravity, setGravity] = useState({ x: 0, y: 1.5 });
  const [deltaTime, setDeltaTime] = useState(1/60);
  const [mousePrev, setMousePrev] = useState({ x: 0, y: 0 });
  ...
  useEffect(() => {
    const loadImages = async () => {
      ...
      for (let i = 1; i <= 5; i++) {
        const img = new Image();
        img.src = `/assets/${i}.png`; // Adjust path based on your project structure
  ...
  const setupParticles = () => {
    ...
    const requiredRows = Math.ceil(particleCount / cols);
      ...
      neighbors.forEach((j: number) => {
        if (i < j) {
          ...
          if (!checkedPairs.has(pairKey)) {
            ...
            const d = Math.sqrt(
              Math.pow(particle.pos.x - other.pos.x, 2) + 
              Math.pow(particle.pos.y - other.pos.y, 2)
            );
            
            if (d < spacing && d > 0) {
              // Collision response logic here if needed
            }
